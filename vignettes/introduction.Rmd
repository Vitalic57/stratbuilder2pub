...
What if you want to check your strategy on diffrent instruments? The package has an answer. Just create list of models. To do that you can make function that return model and insert diffrent data to diffrent objects.
...
stocks <- c("GAZP.ME", "SBER.ME", "LKOH.ME", "ROSN.ME", "NVTK.ME")
...
Most of all reporting, optimization and performing functions working with list as with modelStrategy. 

If we have list of models then we can create portfolio of models. Backtests will be summed inside it and plotPnL will draw summed profit and loss graph.
```{r, warning = FALSE, message = FALSE, fig.width=12, fig.height=6}
